% Binary String to Real Vector conversion (old, better use bin2real)
%
...
%                        use arithmetic and/or logarithmic scaling.
%                lbin,
...
%
% See also: crtbase, crtbp, crtrp, bin2real, bin2int

% Author: Carlos Fonseca   Updated: Andrew Chipperfield   Hartmut Pohlheim
% Date: 08/06/93           Date: 26-Jan-94                Date: 31.05.95
...
% Decode chromosomes
   ...
   expanding = ones(Nind,1);
   if any(scale)
   	Phen(:,scale) = logsgn(expanding,:) .* exp(Phen(:,scale));
   end
