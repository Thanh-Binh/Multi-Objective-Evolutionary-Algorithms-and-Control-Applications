...
% Set default parameters
   Num = 25;                           % Number of figures
   Points = 8;                         % Number of points per figure
...
% Function needs as many rows as columns, every place has to be filled
   ...
   SelectNumber = 3; % ColRowNum;      % Number of parents by selection

   Cols = 1:1:ColRowNum;               % Index of individuals in first columns
   Rows = 1:ColRowNum:Num;             % Index of individuals in first row

   figpatch = figure;                  % Open a new figure for output
   set(figpatch,'NumberTitle','off');  % Switch figure numbering off
   axpat = axes('Position',[0 0 1 1]); % Set axes to whole figure
   set(axpat, 'xcolor',[0 0 0]);       % Make X-Axis invisible
   set(axpat, 'ycolor',[0 0 0]);       % Make Y-Axis invisible
   ...
   for igen = 1:MAXGEN,                % Loop over all generations
      ...
      figure(figpatch);                % Make figpatch current figure
      axes(axpat); cla;                % Make axpat current and clear, removes all objects
      ...
      for isel = 1:SelectNumber,       % Loop for as many points as individuals to choose
         set(figpatch, 'Name',[ '  Select ' int2str(isel) '. figure (of ' ...
