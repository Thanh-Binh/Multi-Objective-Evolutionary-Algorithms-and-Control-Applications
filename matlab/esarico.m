function [NewX, NewS] = esarico(OldX, OldS);
% [NewX, NewS] = esarico(OldX, OldS)
% Evolution Strategy with ARIthmetical CrossOver
%
% This function supports a recombination between one feasible (odd index) 
% and one unfeasible (even index)
% parent to generate a better offspring next to the feasible region
%
% Inputs:
...
Weighting=.75;    % offspring is next to feasible region with the 
...
Alpha = (1-Weighting)*Alpha(ones(nvars,1),1:nmating);
% line-recombination
...
RNeigb=min([1,.5*sum((OldX(:,even) - OldX(:,odd)).^2)]);
Xmutation=RNeigb*(-1+2*rand(nvars,2*nmating));
